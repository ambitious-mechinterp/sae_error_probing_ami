...
from utils import tokenize_data, extract_last_token_acts #type: ignore
...
def train_logistic_probe(
    features: Float[Tensor, "batch d_model"],
...
def train_and_evaluate_probe(
    train_feats: Float[Tensor, "batch d_mod_or_sae_hidden"],
    train_labels: Float[Tensor, "batch"],
    test_feats: Float[Tensor, "batch d_mod_or_sae_hidden"],
    test_labels: Float[Tensor, "batch"],
    feature_type: str,
    seed: int,
    epochs: int,
    batch_size: int,
    ...
    LogisticRegression, dict[str, float | Any], Float[Tensor, "d_mod_or_sae_hidden"]
]:
    """
    ...
    train_loss, train_acc, train_roc_auc, _ = evaluate_probe_full(
        probe, train_feats, train_labels
    )
    test_loss, test_acc, test_roc_auc, _ = evaluate_probe_full(
        probe, test_feats, test_labels
    )
...
def combine_sae_and_error(
    train_indices: NDArray[np.int32],
    test_indices: NDArray[np.int32],
    features_map: dict[str, Float[Tensor, "batch d_mod_or_sae_hidden"]],
    sae: SAE,
    device: t.device | str,
    top_indices: Float[Tensor, "k"],
...
def compute_cosine_similarities(
    weight_vectors: dict[str, Float[Tensor, "d_model"]],
) -> dict[str, float]:
    """
...
def save_probes_and_compute_similarities(
    models_dir: Path,
    results_dir: Path,
    save_probes_count: int,
    result_suffix: str,
        ...
        for i in range(num):
            for j in range(i + 1, num):
                sim = F.cosine_similarity(
                    weights_list[i], weights_list[j], dim=0
                ).item()
                sims.append(sim)
...
def run_probing_pipeline(
    ...
    device: t.device,
    label_col: str,
    features_map: dict[str, t.Tensor],
    models_dir: Path,
    results_dir: Path,
    plot_dir: Path,
    sae: SAE,
...
def process_dataset(dataset, result_suffix, args, model, sae, project_root, device):
    ...
    features_dir = project_root / "data" / "processed" / "tokenized_features"
    features_dir.mkdir(parents=True, exist_ok=True)
    
    # Create a unique filename based on dataset and model parameters
    dataset_name = Path(dataset).stem
    model_name = args.model.replace("-", "_").replace("/", "_")
    sae_id_str = args.sae_id.replace("/", "_").replace("-", "_")
    features_filename = f"{dataset_name}_{model_name}_{sae_id_str}_layer{args.layer}_offset{args.token_position_offset}.pt"
    features_path = features_dir / features_filename
    
    # Check if features already exist
    if features_path.exists():
        print(f"Loading precomputed features from {features_path}")
        features_dict = t.load(features_path)
        features_map = {
            "sae_input": features_dict["sae_input"],
            "sae_diff": features_dict["sae_diff"],
            "sae_acts_post": features_dict["sae_acts_post"],
        }
    else:
        ...
        # Generate features
        print(
            f"Generating features for entire dataset with offset: {args.token_position_offset}"
        )
        feats_all_input, feats_all_recons, feats_all_diff, feats_all_acts_post = (
        ...
        t.save(features_map, features_path)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train logistic regression probes on SAE activations."
    )
    ...
    parser.add_argument(
        "--layer", type=int, default=19, help="Layer to extract activations from"
    )
    parser.add_argument(
        "--sae_id",
        type=str,
        default="layer_19/width_16k/canonical",
...
if __name__ == "__main__":
    ...
    model = HookedSAETransformer.from_pretrained(
        args.model, device="cpu", dtype=t.bfloat16
    )
    ...
    assert len(args.dataset) == len(args.result_suffix), (
        "Dataset and result suffix must have the same length"
    )

    # Process each dataset
    for dataset, result_suffix in zip(args.dataset, args.result_suffix):
        process_dataset(dataset, result_suffix, args, model, sae, project_root, device)
